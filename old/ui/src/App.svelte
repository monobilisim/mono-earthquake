<script lang="ts">
  ...
  let shownMessageRows: MessageRow[] = $derived(
    selectedPoll === '' ? messageRows : messageRows.filter((row) => row.poll_name === selectedPoll)
  );
</script>
        ...
        <div class="header">
          ...
          <div class="logout-div">
            ...
            {#if messageRows[0]?.poll_name}
              <select bind:value={selectedPoll} class="poll-select">
                <option value="">All Polls</option>
                {#if pollNames.length > 0}
                  {#each pollNames as pollName}
                    <option value={pollName}>{pollName}</option>
                  {/each}
                {/if}
              </select>
            {/if}
                ...
                {#each shownMessageRows as row, index}
                  <tr>
                    ...
                    <td class="message-cell">
                      ...
                      {:else}
                        <span class="no-message">No response</span>
                      {/if}
                    </td>
                    <td>
                      {#if row.is_read}
                        <span class="status responded">Read</span>
                      {:else}
                        <span class="status no-response">Not Read</span>
                      {/if}
                    </td>
                    <td>{new Date(row.created_at.replace(' ', 'T') + 'Z').toLocaleString()}</td>
                    {#if row?.poll_name}
                      <td>
